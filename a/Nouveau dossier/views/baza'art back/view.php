...
$val=$_POST['select'];
$cat1c=new CategoryC();
$categorie=$cat1c->remplircat($val);
$menu1c=new SubmenuC();
}
?>
<!doctype html>
...
<body>
    ...
    <!-- Category View
        ============================================ --> 
<form id="regForm" action="modifiercategory.php" method="POST">                  
       <span>Category name:</span> <input id="category" type="text"  name="category" value="<?php echo $categorie->category ?>"  >
        <input type="hidden" name="val" value="<?PHP echo $val; ?>">
       <?PHP $select=$menu1c->remplirchamps($val);
       $i=0;
                        while($s=$select->fetch(PDO::FETCH_OBJ))
           {  $i=$i+1;
                         foreach ($s as $row) { ?>
       <span>submenu name <?PHP echo"($i)" ?>:</span> <input id="submenu" type="text"  name="submenu" value="<?php echo $row ?>"  >
        <?PHP  }} ?>
